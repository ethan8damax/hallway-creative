...
- Andrew's real photos/videos — not yet provided. Site ships with clearly-marked placeholder media in the meantime. Andrew is handing off a hard drive with his library (makes sense — cheaper/faster than uploading everything, and avoids Instagram's heavy compression). The hard drive is just the transfer mechanism, not the final destination: pick the best shots per category from it, then upload those directly into Sanity Studio (`/studio`) — that's Sanity's own asset CDN, which is what actually serves images to the live site. Swapping in real content is a CMS content-entry task, not a code change.
