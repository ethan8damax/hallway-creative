...
- **Subsystem 2 — Client gallery delivery.** Not yet spec'd. Separate concerns: client auth/access, private galleries, native downloads, storage backend, bandwidth/storage cost, pricing model. Will get its own brainstorm → spec → plan cycle when we're ready to build it.
  ...
  - Key open question already discussed: client auth/access model. Leaning toward a unique magic link per gallery (optionally password-protected), matching industry-standard photographer delivery tools (Pixieset, Pic-Time) and Andrew's "ease of download" requirement — no signup friction for a one-time client. Full client accounts would only make sense if clients return across multiple bookings and want a persistent dashboard, which isn't the stated priority. Final call deferred to subsystem 2's spec.
...
- Invite Andrew as a Sanity Studio collaborator so he can upload real photos/videos and write his real bio/hero copy (current copy is placeholder, explicitly not final marketing copy per this file's own rule).
- Final brand visual design pass (Claude design → `/impeccable`), per the standing decision above — this build is structure/functionality only, not final pixels.
